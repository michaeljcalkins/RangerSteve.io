...
export default function() {
  ...
  if (!state.room.id || state.room.state !== "active" || !window.RS.enemies)
    return;

  window.RS.enemies.forEach(enemy => {
    const targetPosition = enemy.data.targetPosition;

    if (!targetPosition || targetPosition.millisRemaining < 0) return;
    ...
    targetPosition.millisRemaining = Math.max(
      targetPosition.millisRemaining - this.game.time.elapsed,
      0
    );
  });
}
