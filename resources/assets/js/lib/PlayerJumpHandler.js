...
export default function PlayerJumpHandler() {
  ...
  if (
    ...
    this.game.store.getState().player.jumpJetCounter >
      GameConsts.JUMP_JET_STARTING_FUEL
  ) {
    ...
    if (
      !jumpjetFxHandle &&
      this.game.store.getState().player.jumpJetCounter >
        GameConsts.JUMP_JET_DEAD_ZONE_FUEL
    ) {
    ...
    store.dispatch(
      actions.player.incrementJumpJetCounter(GameConsts.JUMP_JET_SPEED)
    );
  } else {
    ...
    if (store.getState().player.jumpJetCounter < 0) {
      store.dispatch(
        actions.player.decrementJumpJetCounter(
          GameConsts.JUMP_JET_SPEED_REGENERATION
        )
      );
  ...
  if (
    onTheGround &&
    upInputReleased.call(this, store.getState().game.keyboardControls.up)
  ) {
