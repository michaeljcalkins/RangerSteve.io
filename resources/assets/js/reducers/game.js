...
const initialState = {
  ...
  keyboardControls: {
    ...
    switchWeapon: storage.get(
      "keyboardControl.switchWeapon",
      window.Phaser.Keyboard.Q
    ),
    newChatMessage: storage.get(
      "keyboardControl.newChatMessage",
      window.Phaser.Keyboard.T
    ),
    ...
    selfkill: storage.get(
      "keyboardControl.selfkill",
      window.Phaser.Keyboard.TILDE
    )
  },
...
const player = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "ADD_CHAT_MESSAGE":
      return {
        ...state,
        chatMessages: [...state.chatMessages, action.value].slice(-5)
      };
    ...
    case "ADD_KILL_LOG_MESSAGE":
      return {
        ...state,
        killLogMessages: [...state.killLogMessages, action.value]
      };
