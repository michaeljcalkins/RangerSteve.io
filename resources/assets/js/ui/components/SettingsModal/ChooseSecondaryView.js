...
export function ChooseSecondaryMenu({ onSettingsViewChange, onSecondaryGunClick, onCloseSettingsModal, player }) {
  function handleSelectSecondaryClick(weapon) {
    onSecondaryGunClick(weapon);
    player.health <= 0 ? onCloseSettingsModal() : onSettingsViewChange("main");
  }
  ...
  return (
    <div>
      ...
      <div className="row">
        <div className="col-xs-12">
          <div className="options-menu">{renderWeapons()}</div>
        </div>
      </div>
      <div className="row">
        <div className="col-xs-12">
