...
export class HudLeaderboard extends Component {
  renderPlayers() {
    ...
    return players.map(player => {
      let playerNickname = player.nickname ? player.nickname : "Unnamed Ranger";

      const killingSpreeCount = player.killingSpree > 1 ? `${player.killingSpree}x ` : null;
      ...
      return (
        <tr key={player.id} className={playerRowClasses}>
          <td style={{ width: "120px", overflow: "hidden" }} title="Player's nickname">
            {player.isPremium && (
              <img src="/images/icons/gold-crown.png" width="15" className="mr1" style={{ marginTop: "-4px" }} />
            )}
            {playerNickname}
          </td>
          <td style={{ width: "20px" }} title="Player's current killing spree">
            <strong>{killingSpreeCount}</strong>
          </td>
          <td style={{ width: "20px" }} title="Player's current score">
            {player.score || 0}
          </td>
        </tr>
      );
