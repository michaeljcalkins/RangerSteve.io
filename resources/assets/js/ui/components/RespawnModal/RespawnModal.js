...
export class RespawnModal extends PureComponent {
  ...
  tick() {
    ...
    if ((this.state.autoRespawn || this.state.oneTimeAutoRespawn) && seconds <= 0 && !isRespawning) {
  ...
  renderDamageTaken() {
    ...
    return (
      <div>
        <strong className="text-danger">Damage taken:</strong>
        <strong> {attackingDamage} </strong>
  ...
  renderRespawnButton() {
    if (this.state.elapsed > 0) {
      return (
        <button className="btn btn-primary btn-lg btn-block disabled" onClick={this.handleDisabledRespawnButtonClick}>
    ...
    return (
      <button className="btn btn-primary btn-lg btn-block" onClick={this.handleRespawnButtonClick}>
        Respawn Now
      </button>
    );
  }
  ...
  renderCauseOfDeath() {
    ...
    if (!attackingPlayerId) {
      ...
      return (
        <div className="row">
          <div className="col-xs-12 text-center">
            <img className="weapon-image mb3" src={"/images/guns/large/" + selectedWeapon.image} />
            <h4>
              <strong>{attackingPlayerName}</strong> killed you with their <strong>{selectedWeapon.name}</strong>
            </h4>
            ...
            <div className={modalContentClasses}>
              ...
              <div className="modal-body">
                <div className="row" style={{ margin: "20px 0" }}>
                  <div className="col-xs-6">{this.renderCauseOfDeath()}</div>
                  <div className="col-xs-6 text-center">
                    <div className="mt6">{this.renderRespawnButton()}</div>
