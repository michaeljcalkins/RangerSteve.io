...
module.exports = function(data) {
    ...
    if (!data.attackingPlayerId) {
        console.error('Missing attacking player id.')
        return
    }
    ...
    // Player not found
    if (!damagedPlayer) {
        console.log('Player not found: ' + data.damagedPlayerId)
    ...
    console.log(`Player ${data.attackingPlayerId} damaged ${data.damagedPlayerId} by ${data.damage}.  New health: ${damagedPlayer.health}`)

    if (damagedPlayer.health <= 0) {
        console.log(`Player ${data.attackingPlayerId} killed ${data.damagedPlayerId}`)
