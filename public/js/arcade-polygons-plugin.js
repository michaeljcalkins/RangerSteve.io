...
},{"poly-decomp":1,"sat":2}],4:[function(require,module,exports){
const SAT = require('sat');
  ...
  */
  ...
  for (var i in group.children) {
    ...
    if (collision) {
      ...
      collideCallback && collideCallback.call(sprite, polygon)
...
Overrides.collideGroupVsGroup = function (group1, group2, collideCallback, processCallback, callbackContext, overlapOnly) {
  ...
  */
  ...
  for (var j in group1.children) {
    if (!group1.children[j].body) return false
    if (!group1.children[j].alive) return

    var body = group1.children[j].body

    if (!group1.children[j].body.sat) {
      return console.error('Non polygon object!')
    }
    ...
    for (var i in group2.children) {
      var polygon = group2.children[i];
      ...
      if (collision) {
        ...
        collideCallback && collideCallback.call(group1.children[j], polygon)
...
Phaser.Plugin.ArcadePolygons.prototype.init = function () {
  ...
  this.originalCollideGroupVsGroup = Phaser.Physics.Arcade.prototype.collideGroupVsGroup;
  ...
  Phaser.Physics.Arcade.prototype.collideGroupVsGroup = Phaser.Plugin.ArcadePolygons.Overrides.collideGroupVsGroup;
};
